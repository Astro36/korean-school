...
const getComciganData = school => new Promise((resolve) => {
  for (let i = 0, len = comcigans.length; i < len; i += 1) {
    ...
    if (comcigan.name === school.name) {
      ...
      const { code } = comcigan;
      if (code in cache.comcigan && cache.comcigan[code].date > Date.now() - UPDATE_TIME) {
        resolve(JSON.parse(cache.comcigan[code].data));
        return;
      }
      request.get('http://112.186.146.96:4080/st#', (err, httpResponse, body) => {
        if (err) {
          resolve(null);
        } else {
          const url = body.toString().match(/window.localStorage;var [A-z0-9ㄱ-ㅎ가-힣$_]+\s*=\s*'([\w_-]+)'/)[1];
          request.get({
            url: `http://112.186.146.96:4080/${url}?sc=${code}&nal=1&s=0`,
          }, (err2, httpResponse2, body2) => {
            if (err2) {
              resolve(null);
            } else {
              const data = body2.split('\n')[0];
              cache.comcigan[code] = {
                data,
                date: Date.now(),
              };
...
const getInformation = school => new Promise((resolve) => {
  request.get({
    url: `http://www.schoolinfo.go.kr/ei/ss/Pneiss_b01_s0.do?HG_CD=${school.code}`,
    encoding: null,
