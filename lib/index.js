...
const getComciganData = (school, callback) => {
  for (let i = 0, len = comcigans.length; i < len; i += 1) {
    ...
    if (comcigan.name === school.name) {
      if (comcigan.code in cache.comcigan && cache.comcigan[comcigan.code].date > Date.now() - 10000) {
        callback(cache.comcigan[comcigan.code].data);
        return;
      }
      request.get({
